...
class LeaveController {
	...
	// 				// Step 1: Identify leaveHeaderAutoId where ALL rows are approved
	...
	// 				// Extract leaveHeaderAutoId where all levels are approved
	// 			const excludedLeaveHeaderIds = fullyApprovedLeaveHeaders.map((row) => row.leaveHeaderAutoId);
	...
	// 						},
	// 					],
	// 					...(excludedLeaveHeaderIds.length > 0 && { employeeleaveheaderID: { [Op.notIn]: excludedLeaveHeaderIds } }),
	// 				},
			...
			if (result.status == "approved") {
				for (const leaveID of leaveIds) {
					...
					if (leaveHeaderSingleRecords.leaveAutoId == 9) {
						let returnTrue = await helper.activeCompOffMoreThanLeave(
					...
					if (leaveHeaderSingleRecords.approvalFlowExist == 1) {
						...
						if (leaveTrails.length > 0) {
							...
							await db.leaveApprovalTrails.update(
								...
								{
									where: {
										leaveTrailAutoId: leaveTrails.map(
											(trail) => trail.leaveTrailAutoId,
										),
									},
								},
							);
							...
							if (currentLevel === maxApprovalLevel && existingLeaveHeader) {
								...
								const obj = {
									...
									fromDate: leaveHeaderSingleRecords.fromDate,
									toDate: leaveHeaderSingleRecords.toDate,
									leaveType:
									...
									await db.attendanceMaster.update(
										Object.assign(
											SingleexistingRecordSWholes.dataValues.isHalfDay === 0
												...
												: SingleexistingRecordSWholes.dataValues.halfDayFor ===
													  1
													? { attendanceLateBy: "00:00:00" }
													: {},
										),
										...
										await db.leaveMapping.increment(
											{
												utilizedThisYear: parseFloat(
													existingLeaveHeader.leaveCount,
												),
											},
											{
												where: {
	...
	async requestForLeave(req, res) {
		try {
			...
			const leaveApprovalLevel = await db.leaveApprovalLevel.findAll({
				where: {
				...
				for (const leaveApproverGroup of leaveApprover.dataValues.approval_group.split(
					...
					if (leaveApproverGroup === "MANAGER") {
						...
						leaveTrails.push({
							...
							isVisible: leaveApprover.dataValues.level === 1 ? 1 : 0,
							pendingOn: EMP_DATA.managerData.id,
							isApproved: 0,
							isPending: 1,
							isActive: 1,
							createdAt: moment(),
						...
						for (const buHrIds of buhr) {
							leaveTrails.push({
								...
								approvalFlowAutoId: leaveMasterData.approvalFlow,
								isVisible: leaveApprover.dataValues.level === 1 ? 1 : 0,
								pendingOn: buHrIds.dataValues.buHrId,
								isApproved: 0,
								isActive: 1,
								createdAt: moment(),
								createdBy: req.userId,
						...
						leaveTrails.push({
							...
							approvalFlowAutoId: leaveMasterData.approvalFlow,
							isVisible: leaveApprover.dataValues.level === 1 ? 1 : 0,
							pendingOn: EMP_DATA.managerData.manager,
							isApproved: 0,
							isActive: 1,
							createdAt: moment(),
							createdBy: req.userId,
		...
		try {
			...
			if (result.status == "approved") {
				for (const leaveID of leaveIds) {
					...
					if (existingRecordNew.leaveAutoId == 9) {
						let returnTrue = await helper.activeCompOffMoreThanLeave(
							existingRecordNew.employeeId,
							leaveID,
						);
					...
					if (existingRecordNew.approvalFlowExist == 1) {
						...
						if (req.userData.role_id == 2 || req.userData.role_id == 5) {
							...
							const isFinalApproval = leaveTrails.some(
								(trail) => trail.level === trail.approval_flow.maxApprovalLevel,
							);
							...
							const existingLeaveHeaderRecord =
								await db.EmployeeLeaveHeader.findOne({
									where: { employeeleaveheaderID: leaveID },
								});
							...
							for (const trail of leaveTrails) {
								await db.leaveApprovalTrails.update(
									...
									{
										where: {
											leaveTrailAutoId: trail.leaveTrailAutoId,
											isApproved: 0,
										},
									},
								);
							...
							if (existingRecord) {
								...
								if (
									...
									existingLeaveHeaderRecord.leaveAutoId === 9
								) {
									const lwpLeave = await db.leaveMapping.findOne({
										where: {
									...
									if (lwpLeave) {
										await db.leaveMapping.increment(
											{
												utilizedThisYear: parseFloat(
													existingLeaveHeaderRecord.leaveCount,
												),
											},
											{
												where: {
										...
										await db.attendanceMaster.update(
											Object.assign(
												SingleexistingRecordSWholes.dataValues.isHalfDay === 0
													...
													: SingleexistingRecordSWholes.dataValues
																.halfDayFor === 1
														? { attendanceLateBy: "00:00:00" }
														: {},
											),
											{
												where: {
													attendanceDate:
														SingleexistingRecordSWholes.appliedFor,
													employeeId: SingleexistingRecordSWholes.employeeId,
												},
											},
										);
