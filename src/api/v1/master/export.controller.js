	...
	data.forEach((item, index) => {
		...
		if (!groupedData[employeeId]) {
			groupedData[employeeId] = {
				"Employee Id": employeeId, //1
				"Employee Name": item["Employee Name"], //2
				"Date of Joining": item["Date of Joining"]?moment(item["Date of Joining"]).format("DD-MM-YYYY"):'N/A', //3
				"Exit Date":item["Exit Date"]?moment(item["Exit Date"]).format("DD-MM-YYYY"):'N/A' , //4
				"Total Days": item["Total Days"], //5
				"LOP Days": item["LOP Days"], //6
				"Arrears Days": item["Arrears Days"], //7
				"Present Days": item["Present Days"] ? item["Present Days"] : 0, //8
...
const transformData = (data) => {
	return data.map((employee) => {
		let transformedObj = {
			...
			"Date Of Joining": moment(new Date(employee.employeejobdetail.dateOfJoining)).format('DD-MM-YYYY'),
		};
