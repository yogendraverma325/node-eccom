...
class ThirdPartyController {
  async employeeDatadata(req, res) {
    try {
      ...
      if (isValid && isActive == 1) {
        const employeeData = await db.employeeMaster.findAll({
          where: { isActive: isActive, employeeType: 3 },
          attributes: [
            "id",
            "empCode",
            "email",
  ...
  async employeeData(req, res) {
    try {
      ...
      // console.log("Unix Timestamp (seconds):", unixTimeSeconds);

      // // Round the current Unix timestamp to the nearest 5-minute interval (300 seconds)
      // const roundedUnixTime = Math.floor(unixTimeSeconds / 300) * 300;

      // console.log("Rounded Unix Time (nearest 5 minutes):", roundedUnixTime);

      // // Function to generate a hash
      // async function generateHash(email, secretKey, time) {
      //   const concatenatedString = `${email}${secretKey}${time}`;
      ...
      // // Generate the hash for the current time (rounded to the nearest 5 minutes)
      // const generatedHash = await generateHash(taraEmailId, taraSecretKey, roundedUnixTime);

      // // Now, this hash is valid for 5 minutes, starting from the roundedUnixTime
      // console.log("Generated Hash (valid for 5 minutes):", generatedHash);

      // // Set the time window (5 minutes window in seconds)
      // const timeWindowInSeconds = 5 * 60;

      // // Check hashes for all times in the window (from roundedUnixTime to roundedUnixTime + 5 minutes)
      // for (let i = 0; i < timeWindowInSeconds; i++) {
      //   const timeToCheck = roundedUnixTime - i;
      //   const validHash = await generateHash(taraEmailId, taraSecretKey, timeToCheck);

      //   if (crypto.timingSafeEqual(Buffer.from(validHash, "hex"), Buffer.from(generatedHash, "hex"))) {
