...
class AttendanceController {
		...
		try {
			...
			await db.regularizationMaster.create({
				...
				creatorRole: helper.fetchEmployeeRole(
					req.userRole,
					attendanceData.dataValues.employeeId,
					req.userId,
				),
				attendanceShiftStartDate:
					attendanceData.dataValues.attandanceShiftStartDate,
				attendanceShiftEndDate:
				...
				await db.regularizationMaster.update(
					{
						...
						updatorRole: helper.fetchEmployeeRole(
							req.userRole,
							regularizeData["attendancemaster.employeeId"],
							req.userId,
						),
					},
					{
					...
					await db.attendanceMaster.update(
						{
							...
							attendanceLateBy: await helper.calculateLateBy(
								...
								withGraceTime,
								regularizeData["attendancemaster.attendanceDate"],
						...
						if (
							...
							if (
								...
								if (totalMinutesLateMinutes > 0) {
									totalMinutesLateMinutes =
										totalMinutesLateMinutes +
										singleEmp.attendancemaster.attendancePolicymaster
											.graceTimeClockIn; // Adjust Grace time with late by for leave calculation
								}
								...
								if (EMP_DATA) {
									await helper.empMarkLeaveOfGivenDate(
										singleEmp.id,
										{
											...
											attendancePolicyId:
												singleEmp.attendancemaster.attendancePolicymaster
													.attendancePolicyId, // Replace with actual attendance policy ID
											leaveAutoId:
												singleEmp.attendancemaster.attendancePolicymaster
													.leaveDeductPolicyLateDurationLeaveType, // Replace with actual leave auto ID
							...
							if (
								(singleEmp.attendancemaster.attendancePunchInTime &&
									!singleEmp.attendancemaster.attendancePunchOutTime &&
									singleEmp?.weekOffMaster &&
									singleEmp?.weekOffMaster?.weekOffDayMappingMasters.length >
										0) ||
								(singleEmp.attendanceroster &&
									singleEmp.attendanceroster.weekOffMaster &&
									singleEmp.attendanceroster.weekOffMaster
	...
	async revokeApprovedRegularizations(req, res) {
		try {
			...
			let attendanceData = await db.attendanceMaster.findOne({
				...
				include: [
					// adding association shift and policy to calcualte late by and its policy
					{
						model: db.shiftMaster,
						required: false,
						attributes: [
			...
			await helper.releaseCompOffTheEmployeeForDate(
			...
			if(attendanceData.attendanceDate!=moment().format("YYYY-MM-DD")){ // added check , not to run cron , if attendance is being revoke on current date
				await _this.attedanceCronManual(
	...
	async runcronforEMPForTheGivenDate(req, res){
		const empIds = req.body.empIds;
		const dates = req.body.dates;
		for (const empId of empIds) {
		for(const date of dates){
		
					await _this.makeAttendance(
					date,
					empId
					);
		}
	...
	async makeAttendance(date,empId){
	...
	async runcronforEMPForThejoiningDate(req, res){
		const fromDate = req.body.fromDate;
		const toDate = req.body.toDate;

		const EMP_DATAS = await db.employeeMaster.findAll({
				where: {
					isActive: 1,
					companyId:{
						[Op.notIn]:[1,4]
				...
				include: [
					{
						...
						where:{
						dateOfJoining: {
						[Op.between]: [fromDate,toDate] // Replace with your actual range
						}
						}
					},
				]});

			console.log("EMP",EMP_DATAS.length)
		for (const SINGLE_EMP of EMP_DATAS) {
			 let dates_ranges=await helper.getDatesArray(SINGLE_EMP.employeejobdetail.dateOfJoining,moment().format("YYYY-MM-DD"));
			for(const date of dates_ranges){
				const checkAttendance = await db.attendanceMaster.findOne({
					where: {
						employeeId: SINGLE_EMP.id,
						attendanceDate: date,
					},
				});
				if(!checkAttendance){
