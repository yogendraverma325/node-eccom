...
async function query(caseId, data, data2) {
	switch (caseId) {
		...
		case 22:
			...
			break;
			case 23:
			//return `SELECT SUM(ptAmount) AS ptAggregateAmount, GROUP_CONCAT(empCode) AS ptImpactedEmployees FROM tara_hrms_live.ptoverrides WHERE EmployeeId IN (${data.impactedEmployees}) AND ptMonth = '${data.deductionMonth}' CROSS JOIN (SELECT SUM(lwfAmount) AS lwfAggregateAmount, GROUP_CONCAT(empCode) AS lwfImpactedEmployees FROM tara_hrms_live.lwfoverrides WHERE EmployeeId IN (${data.impactedEmployees}) AND lwfMonth = '${data.deductionMonth}') AS lwf CROSS JOIN (SELECT SUM(recoveryDays) AS noticeAggregateDays, GROUP_CONCAT(empCode) AS noticeImpactedEmployees FROM tara_hrms_live.noticerecoveryovrrides WHERE EmployeeId IN (${data.impactedEmployees}) AND payMonth = '${data.deductionMonth}') AS nr;`; //14
			//return `SELECT pt.ptAggregateAmount, pt.ptImpactedEmployees, lwf.lwfAggregateAmount, lwf.lwfImpactedEmployees, nr.noticeAggregateDays, nr.noticeImpactedEmployees FROM (SELECT SUM(ptAmount) AS ptAggregateAmount, GROUP_CONCAT(empCode) AS ptImpactedEmployees FROM tara_hrms_live.ptoverrides WHERE EmployeeId IN (${data.impactedEmployees}) AND ptMonth = '${data.deductionMonth}') AS pt CROSS JOIN (SELECT SUM(lwfAmount) AS lwfAggregateAmount, GROUP_CONCAT(empCode) AS lwfImpactedEmployees FROM tara_hrms_live.lwfoverrides WHERE EmployeeId IN (${data.impactedEmployees}) AND lwfMonth = '${data.deductionMonth}') AS lwf CROSS JOIN (SELECT SUM(recoveryDays) AS noticeAggregateDays, GROUP_CONCAT(empCode) AS noticeImpactedEmployees FROM tara_hrms_live.noticerecoveryovrrides WHERE EmployeeId IN (${data.impactedEmployees}) AND payMonth = '${data.deductionMonth}') AS nr;`
			 return `SELECT    ue.uniqueEmployeeCounts,pt.ptAggregateAmount, pt.ptImpactedEmployees, lwf.lwfAggregateAmount, lwf.lwfImpactedEmployees, nr.noticeAggregateDays, nr.noticeImpactedEmployees, ed.extraDeductionAggregateAmount, ed.extraDeductionImpactedEmployees, tds.tdsAggregateAmount, tds.tdsImpactedEmployees FROM (SELECT SUM(ptAmount) AS ptAggregateAmount, GROUP_CONCAT(empCode) AS ptImpactedEmployees FROM tara_hrms_live.ptoverrides WHERE EmployeeId IN (${data.impactedEmployees}) AND ptMonth = '${data.deductionMonth}') AS pt CROSS JOIN (SELECT SUM(lwfAmount) AS lwfAggregateAmount, GROUP_CONCAT(empCode) AS lwfImpactedEmployees FROM tara_hrms_live.lwfoverrides WHERE EmployeeId IN (${data.impactedEmployees}) AND lwfMonth = '${data.deductionMonth}') AS lwf CROSS JOIN (SELECT SUM(recoveryDays) AS noticeAggregateDays, GROUP_CONCAT(empCode) AS noticeImpactedEmployees FROM tara_hrms_live.noticerecoveryovrrides WHERE EmployeeId IN (${data.impactedEmployees}) AND payMonth = '${data.deductionMonth}') AS nr CROSS JOIN (SELECT SUM(deductionAmount) AS extraDeductionAggregateAmount, GROUP_CONCAT(empCode) AS extraDeductionImpactedEmployees FROM tara_hrms_live.extradeductions WHERE EmployeeId IN (${data.impactedEmployees}) AND startMonth = '${data.deductionMonth}') AS ed CROSS JOIN (SELECT SUM(tdsAmount) AS tdsAggregateAmount, GROUP_CONCAT(empCode) AS tdsImpactedEmployees FROM tara_hrms_live.tdsdeductions WHERE EmployeeId IN (${data.impactedEmployees}) AND tdsMonth = '${data.deductionMonth}') AS tds CROSS JOIN (SELECT COUNT(DISTINCT EmployeeId) AS uniqueEmployeeCounts FROM (SELECT EmployeeId FROM tara_hrms_live.ptoverrides WHERE EmployeeId IN (${data.impactedEmployees}) AND ptMonth = '${data.deductionMonth}' UNION SELECT EmployeeId FROM tara_hrms_live.lwfoverrides WHERE EmployeeId IN (${data.impactedEmployees}) AND lwfMonth = '${data.deductionMonth}' UNION SELECT EmployeeId FROM tara_hrms_live.noticerecoveryovrrides WHERE EmployeeId IN (${data.impactedEmployees}) AND payMonth = '${data.deductionMonth}' UNION SELECT EmployeeId FROM tara_hrms_live.extradeductions WHERE EmployeeId IN (${data.impactedEmployees}) AND startMonth = '${data.deductionMonth}' UNION SELECT EmployeeId FROM tara_hrms_live.tdsdeductions WHERE EmployeeId IN (${data.impactedEmployees}) AND tdsMonth = '${data.deductionMonth}') AS combined_ids) AS ue;`
			break;	
	}
}
