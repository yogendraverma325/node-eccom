...
async function query(caseId, data, data2) {
  switch (caseId) {
    ...
    case 3:
      ...
      break;
    case 4:
      return `SELECT pm.name AS process_name, pm.payMonth, pm.payProcessMasterAutoId as processId , pm.createdAt, pm.updatedBy, pm.updatedAt, pf.currentstatus, pf.nextstatus, pf.refferenceFlowId, pf.endOfFlow, ps.name AS status_name, ps.description AS status_description, e.name AS created_by_name, cm.companyName AS company_name, COUNT(pd.proceessId) AS process_details_count FROM payprocessmaster pm INNER JOIN payprocessflowmaster pf ON pm.processFlowId = pf.payProcessFlowMasterAutoId INNER JOIN paystatusmaster ps ON pf.currentstatus = ps.payProcessStatusAutoId LEFT JOIN employee e ON pm.createdBy = e.id INNER JOIN companymaster cm ON pm.companyId = cm.companyId LEFT JOIN payprocessdetails pd ON pm.payProcessMasterAutoId = pd.proceessId WHERE ps.payProcessStatusAutoId IN (${data}) AND pm.payMonth = "${data2.paymonth}" AND pm.companyId = ${data2.companyId} GROUP BY pm.payProcessMasterAutoId`;
      break;
    case 5:
      return `SELECT pf.nextstatus, pf.refferenceFlowId, pf.endOfFlow, pf.currentstatus, ps.name AS status_name, ps.description AS status_description FROM payprocessmaster pm INNER JOIN payprocessflowmaster pf ON pm.processFlowId = pf.payProcessFlowMasterAutoId INNER JOIN paystatusmaster ps ON pf.nextstatus = ps.payProcessStatusAutoId WHERE pm.payProcessMasterAutoId = ${data};`;
      break;
